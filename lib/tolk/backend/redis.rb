...
module Tolk
  module Backend
    ...
    #
    #   Tolk::Backend::KeyValue.new(@store, false)
    #
    ...
    class Redis
      module Implementation
        ...
        def store_translations(locale, data, options = {})
          ...
          flatten_translations(locale, data, escape, @subtrees).each do |key, value|
            key = Tolk.phrase_lookup_key

            case value
            when Hash
              if @subtrees && (old_value = @store.get(key))
            ...
            @store.hset(key, ActiveSupport::JSON.encode(value), 'hrm') unless value.is_a?(Symbol)
          end
        end

        def available_locales
          locales = @store.keys.collect { |k| k =~ /\./; $` }
          locales.uniq!
          locales.compact!
          locales.map(&:to_sym)
        end
      ...
      protected
        ...
        def lookup(locale, key, scope = [], options = {})
          lookup = Tolk.lookup_value_key(normalize_flat_keys(locale, key, scope, options[:separator]))
          value = @store.hget(Tolk.phrase_lookup_key, key)
